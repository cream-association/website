  ...
  <div class="drawer" v-show="drawerStore.isOpen" ref="sidebar">
    <div class="drawer__top">
      <div class="drawer__logo hoverable">
        <a @click="navigateTo(switchLocalPath('/', locale))" class="drawer__anchor">
          <Image :source="Logo" alt-text="application logo" />
        </a>
      </div>
      <Button variant="outline" size="icon" class="drawer__close-btn" @click="drawerStore.toggleDrawer()"
        v-show="drawerStore.isOpen" v-if="isTabletOrMobile()">
        <Cross1Icon class="drawer__close-btn-icon" />
      </Button>
    </div>
    <nav class="drawer__nav">
      <NuxtLinkLocale activeClass="drawer__nav-item-active" class="drawer__nav-item hoverable cursor-pointer" to="/">
        {{ $t("navigation.home") }}
      </NuxtLinkLocale>
      <NuxtLinkLocale activeClass="drawer__nav-item-active" class="drawer__nav-item hoverable cursor-pointer"
        to="/about">
        {{ $t("navigation.about") }}
      </NuxtLinkLocale>
      <NuxtLinkLocale activeClass="drawer__nav-item-active" class="drawer__nav-item hoverable cursor-pointer"
    ...
    <footer class="drawer__footer gap-2">
      <div class="items-top flex gap-x-2 mb-2">
        <Checkbox id="custom-cursor" v-model:checked="globalStore.useCustomCursor" />
